...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM movie left join writer on movie.writer_id = writer.id WHERE movie.id={$filtered_id}";
    $result = mysqli_query($link, $query);
...
<body>
  ...
  <ol> <?=$list ?> </ol>
  <a href="create.php">다른 영화 추가하기</a>
  <?=$update_link?>
  <?=$delete_link?>
  <h2> <?= $article['title'] ?> </h2>
  <?= $article['description'] ?>
